    ...
    for(size_t step=1; nextMeasurement < measurements.size(); ++step) {
        ...
        while(nextMeasurement < measurements.size()) {
                    ...
                    boost::dynamic_pointer_cast<BetweenFactor<Pose> >(measurementf))
            {
                ...
                Key larger = max(measurement->key1(), measurement->key2());
                Key smaller = min(measurement->key1(), measurement->key2());
                // Initialize the new variable
                if (!newVariables.exists(larger)) {
                    Pose newPose;
                    if(step == 1) {
                        newPose = measurement->measured().inverse();
                    }
                    else {
                        if(isam2.valueExists(smaller)) {
                            prevPose = isam2.calculateEstimate<Pose>(smaller);
                        }
                        newPose = prevPose * measurement->measured().inverse();
                    }
                    newVariables.insert(larger, newPose);
                    prevPose = newPose;
