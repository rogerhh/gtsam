...
int main(int argc, char *argv[]) {
    ...
    for(size_t step=1; nextMeasurement < measurements.size(); ++step) {
        ...
        if(K_count == K || nextMeasurement == measurements.size()) {
            ...
            isam2.update(newFactors, newVariables, params, vio_tree_depth, vio_tree_depth, vio_relin_keys);
            ...
            if(step >= num_steps) {
              ...
              while(1) {
                ...
                auto start = chrono::high_resolution_clock::now();
                isam2.update(dummy_nfg, dummy_vals, relin_tree_depth, max_relin_factors, max_relin_keys);
                auto update_end = chrono::high_resolution_clock::now();
                estimate = isam2.calculateEstimate();
                auto calc_end = chrono::high_resolution_clock::now();
                ...
                cout << "iter = " << iter << ", step = " << step << ", Chi2 = " << last_chi2 
                  << ", graph_error = " << isam2.getFactorsUnsafe().error(estimate) << endl;

                // estimate.print();

                if(fabs(last_chi2 - chi2) < d_error) {
                  break;
                }
